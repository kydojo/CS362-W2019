...
int main() {
	...
	if( TESTING ) { printf( "TESTING CARD: great hall\n" ); }
	...
	cardEffect( great_hall, 0, 0, 0, &newState, 0, 0 );
	...
	// *** Test that the player's actions increase by 1 ***
	if( TESTING ) { 
		printf( "\t\tPlayer Gains 1 Actions" );
		if( newState.numActions == origState.numActions + 1 ) {
			printf("\t\tPASS\n");
		}
		else {
			printf("\t\tFAIL\n");		
